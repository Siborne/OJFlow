...
class AtCoderAdapter extends base_adapter_1.BaseAdapter {
    id = 'atcoder';
    displayName = 'AtCoder';
    contestUrl = 'https://atcoder.jp/contests/';
    userUrl = 'https://atcoder.jp/users/';
    kenkooooUrl = 'https://kenkoooo.com/atcoder/atcoder-api/v3/user/ac_rank';
    async fetchContests(days) {
        ...
        rows.each((_i, el) => {
            try {
                ...
                if (!Number.isFinite(startTime) || startTime < 1000000000)
                    return;
                const parts = durationText.split(':').map(Number);
                const duration = (parts[0] || 0) * 3600 + (parts[1] || 0) * 60;
                if (startTime + duration < todayStart || startTime > queryEnd)
                    return;
                if (duration >= 86400)
                    return;
