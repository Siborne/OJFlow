...
## 引言
本开发指南面向新加入的开发者，提供从环境搭建到日常开发、测试、调试与质量保障的完整流程说明。项目采用 Electron + Vue 3 + TypeScript + Vite + Bun 的现代化技术栈，结合 Playwright 进行端到端测试，配合 ESLint/Prettier 与类型检查保障代码质量。
...
- 打包后页面空白
  ...
  - 参考路径：[vite.config.ts](file://vite.config.ts#L6)
- Electron 开发/生产加载异常
...
## 结论
本指南提供了从环境搭建到开发、测试、调试与质量保障的全流程说明。建议新开发者优先完成环境准备与基础脚本运行，再逐步深入前端应用与 Electron 主进程的职责边界，配合测试与类型检查持续提升代码质量与稳定性。
...
- ESLint 与 Prettier
  - ESLint 脚本覆盖 src、electron、shared、tests 目录；Prettier 统一格式
  - 参考路径：[package.json:50-53](file://package.json#L50-L53)
...
### 代码审查清单与质量保证
- 代码审查清单
  - 是否通过 ESLint/Prettier 格式化与检查
  - 是否通过类型检查与单元/端到端测试
  - 是否新增必要的 IPC 参数校验与错误分类
  - 是否更新相关文档与变更说明
  - 参考路径：[package.json:50-53](file://package.json#L50-L53), [tests/e2e/app.spec.ts:1-190](file://tests/e2e/app.spec.ts#L1-L190), [tests/unit/layout.test.ts:1-107](file://tests/unit/layout.test.ts#L1-L107)
- 质量保证流程
